 ...
 *
 * A filter sidebar component specifically designed for collection pages.
 * Excludes the collection facet (since we're already filtering by collection)
 * and shows categories, tags, price range, and material facets.
 *
 * @module app/components/search/CollectionFilters
 */
...
/**
 * Sort options for collection product listing
 */
const SORT_OPTIONS = [
...
/**
 * Props for the CollectionFilters component
 */
...
const CustomSortingView: FC<ElasticSortingViewProps> = ({ className, label, onChange, options, value }) => {
  return (
    <div className={clsx('flex items-center gap-2', className)}>
      <label htmlFor="collection-sort-select" className="text-sm font-medium text-gray-700">
        {label || 'Sort by'}
      </label>
      <select
 ...
 * CollectionFilters Component
 *
 * A filter panel specifically for collection pages that includes:
 * - Sort selector with sorting options
 * - Category facet (since we're on a collection page)
 * - Tags facet
 * - Price range facet
 * - Material and other metadata facets
 *
 * Note: Collection facet is intentionally excluded since we're already
 * filtering by collection on this page.
 *
 * @example
 * ```tsx
 * import { CollectionFilters } from "@app/components/search"
 *
 * function CollectionPage() {
 *   return (
 *     <CollectionSearchProvider collectionId="col_123">
 *       <div className="flex">
 ...
 * ```
 */
export const CollectionFilters: FC<CollectionFiltersProps> = ({
  className,
  showSort = true,
  showCategories = true,
  showTags = true,
 ...
 */
export { SORT_OPTIONS as COLLECTION_SORT_OPTIONS };
