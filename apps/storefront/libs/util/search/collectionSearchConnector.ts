/**
 * Collection Search Connector
 *
 * Custom connector for collection pages that pre-applies a collection filter
 * and routes requests through the Medusa backend browse API.
 * Excludes collection facet from results since we're already filtering by collection.
 *
 * @module libs/util/search/collectionSearchConnector
 */
...
/**
 * Configuration options for the collection search connector
 */
export interface CollectionSearchConnectorConfig {
 ...
 * Excludes collection_names facet since we're already filtering by collection
 */
function transformFacets(apiFacets: Record<string, FacetResult> | undefined): ResponseState['facets'] {
  ...
  for (const [field, facetResult] of Object.entries(apiFacets)) {
    // Skip collection_names facet - we're already filtering by collection
    if (field === 'collection_names') {
      continue;
    }
 ...
 * Merges user-selected filters with the pre-applied collection filter
 */
function buildFilters(filters: RequestState['filters'], collectionId: string): Record<string, unknown> {
  const filterObj: Record<string, unknown> = {
    // Always include the collection filter
...
/**
 * Creates a collection search connector that routes requests through
 * the Medusa backend browse API with a pre-applied collection filter.
 *
 * @param config - Configuration options
 * @returns APIConnector instance for use with SearchProvider
 */
export function createCollectionSearchConnector(config: CollectionSearchConnectorConfig): APIConnector {
  const { collectionId, backendUrl, publishableApiKey, regionId } = config;
  ...
  return {
      ...
      try {
        const response = await fetch(`${backendUrl}/store/search/browse`, {
          ...
          body: JSON.stringify({
            size: resultsPerPage,
            from,
            filters: buildFilters(state.filters, collectionId),
            regionId: regionId || undefined,
          }),
        });

        if (!response.ok) {
          console.error('Collection browse failed:', response.statusText);
          return {
            ...emptyResponse,
            wasSearched: true,
          };
        ...
        return {
          ...
          totalResults,
          totalPages,
          requestId: `collection-browse-${Date.now()}`,
          wasSearched: true,
        ...
        console.error('Collection browse failed:', error);
        return {
          ...emptyResponse,
          wasSearched: true,
        };
    ...
    async onAutocomplete(): Promise<AutocompleteResponseState> {
      // Autocomplete is not used on collection pages
...
export default createCollectionSearchConnector;
